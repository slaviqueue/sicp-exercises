...
; the currency used by the program, so that we could compute the number of ways to change a British
; pound, for example. As the program is written, the knowledge of the currency is distributed partly
; into the procedure first-denomination and partly into the procedure count-change (which knows that
; there are five kinds of U.S. coins). It would be nicer to be able to supply a list of coins to be
; used for making change.


; We want to rewrite the procedure cc so that its second ar- gument is a list of the values of the
