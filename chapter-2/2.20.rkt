...
; list that has a dot before the last pa- rameter name indicates that, when the procedure is called,
; the initial parameters (if any) will have as values the initial arguments, as usual, but the final
...
; then in the body off, x will be 1, y will be 2, and z will be
...
(define (same-parity n . rest)
  (define is-n-odd? (odd? n))
  (define filtered-rest
    (filter (lambda (n) (eq? is-n-odd? (odd? n)))
            rest))
  (cons n filtered-rest))
